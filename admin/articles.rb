...
ActiveAdmin.register Goldencobra::Article, :as => "Article" do
  ...
  filter :article_type, :as => :select, :collection => Goldencobra::Article.article_types_for_select.map{|s| [I18n.t(s.parameterize.underscore.downcase, scope: [:goldencobra, :article_types], default: "#{s}"), s]}.sort, :label => I18n.t("filter_type", :scope => [:goldencobra, :filter], :default => "Artikeltyp")
