ActiveAdmin.register Goldencobra::Article, :as => "Article" do
  ...
  Goldencobra::Article.article_types_for_select.each do |article_type|
    ...
    scope(I18n.t(article_type.split(' ').first, :scope => [:goldencobra, :article_types])){ |t| t.where("article_type LIKE '%#{article_type.split(' ').first}%'") }
  end  
  ...
  form :html => { :enctype => "multipart/form-data" }  do |f|
    ...
    if f.object.new_record?
      ...
      f.inputs "Allgemein", :class => "foldable inputs" do
        ...
        f.input :frontend_tag_list, hint: "Frontend Tags werden u.a. f&uuml;r die Filterung von &Uuml;bersichtsseiten benutzt. (komma-getrennt)", label: "Frontend Tags"
      end
      ...
      f.inputs "Einstellungen", :class => "foldable closed inputs" do
        ...
        f.input :parent_id, :as => :select, :collection => Goldencobra::Article.all.map{|c| ["#{c.path.map(&:title).join(" / ")} => #{c.title}", c.id]}, :include_blank => true, :hint => "Auswahl des Artikels, der in der Seitenstruktur _oberhalb_ liegen soll. Beispiel: http://www.meine-seite.de/der-oberartikel/mein-artikel"
