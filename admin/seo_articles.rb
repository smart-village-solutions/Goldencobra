  ...
  index download_links: proc { Goldencobra::Setting.for_key("goldencobra.backend.index.download_links") == "true" }.call do
    ...
    column I18n.t('active_admin.seo_articles.index.links_column') do |article|
      if article.link_checker.present?
        "#{article.link_checker.count} / E:#{article.link_checker.count - article.link_checker.count { |_key, value| value['response_code'] == "200" }}"
      end
    end
    ...
    column "" do |article|
      ...
      result += link_to(t(:new_subarticle), new_admin_article_path(parent: article.id), class: "member_link edit_link new_subarticle", title: I18n.t('active_admin.seo_articles.column.title3'))
