...
module Goldencobra
  class Articletype < ActiveRecord::Base
    ...
    ArticleFieldOptions = {
        ...
        parent_id: %{<% f.input :parent_id, hint: I18n.t("goldencobra.article_field_hints.parent_id"), as: :select, collection: Goldencobra::Article.where("id = ?", f.object.parent_id).map{ |a| [a.parent_path, a.id] }.sort, include_blank: true, input_html: { class: 'get_goldencobra_articles_per_remote', style: 'width: 70%;', 'data-placeholder' => 'Elternelement auswählen' } %>},
        ...
        article_images: %{<% f.has_many :article_images do |ai|
          ai.input :image, as: :select, collection: Goldencobra::Upload.where(id: ai.object.image_id).map{ |c| [c.complete_list_name, c.id] }, input_html: { class: 'article_image_file chzn-select-deselect get_goldencobra_uploads_per_remote', style: 'width: 70%;', 'data-placeholder' => 'Bitte warten' }, label: "Medium wählen", include_blank: false
