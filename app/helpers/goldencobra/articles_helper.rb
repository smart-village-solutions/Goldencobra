  ...
  module ArticlesHelper
    ...
    def navigation_menu(menue_id, options={})
      ...
      if master_menue
        ...
        result = content_tag(:ul, raw(content),:id => "#{id_name}", :class => "#{class_name} #{depth} #{content.present? ? 'has_elements' : 'empty'} navigation #{master_menue.css_class.gsub(/\W/,' ')}".squeeze(' ').strip)
      end
