  ...
  module Api
    module V2
      class ArticlesController < ActionController::Base
        ...
        def index
          if params[:article_ids].present?
            ...
            respond_to do |format|
              format.json { render json: Oj.dump({"articles" => articles_as_json}, mode: :compat) }
        ...
        def find_article
          @article = Goldencobra::Article.where(creator_id: current_user.id,
                                               external_referee_id: params[:referee_id]).first
        ...
        def author
          if params[:author].present? && params[:author][:lastname].present?
            Goldencobra::Author.find_or_create_by_lastname(params[:author][:lastname])
          else
            nil
          end
        end
        ...
        def create_article(article_param)
          ...
          article_param.each{ |i| new_article.images << Goldencobra::Upload.create(i[:image]) }
          new_article.save
          new_article
        end
        ...
        def articles_as_json
          if params[:react_select] && params[:react_select] == "true"
            ...
            @articles.map { |a| a.as_json(only: [:id, :title], methods: [:parent_path]) }
          end
        end
        ...
        def cached_articles
          ...
          Rails.cache.fetch(cache_key) do
            Goldencobra::Article.select([:id, :title, :ancestry]).sort { |a, b| a[0] <=> b[0] }
          end
        end
      end
    end
