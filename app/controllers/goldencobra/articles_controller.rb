module Goldencobra
  class ArticlesController < Goldencobra::ApplicationController
        ...
        if @article.article_type.present? && @article.article_type_form_file != "Default" && @article_type = @article.send(@article.article_type_form_file.downcase.to_sym)
          ...
          if @article.sort_order.present?
            if @article.sort_order == "Random"
              ...
              @list_of_articles = @list_of_articles.flatten.sort_by!{|article| sort_order.inject(article){|result, message| result.send message} }
            end
    ...
    def convert_to_pdf
      ...
      uri = URI.parse("http://html2pdf.ikusei.de/converter/new.xml?name=#{@article.absolute_public_url}&uid=#{uid}&url=#{@article.absolute_public_url}#{CGI::escape('?pdflayout=true')}")
      logger.debug(uri)
